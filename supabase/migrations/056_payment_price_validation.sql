-- Migration: 056_payment_price_validation.sql
-- Purpose: Server-side price validation for payments (anti-fraude).
--          Before this migration the RPC trusted the client's unit_price,
--          descuento and created_by. Now:
--          1. unit_price is validated against the services catalog
--             (floor = precio_minimo for variable services, precio_base for
--             fixed services; ceiling = precio_maximo when defined).
--          2. Items linked to an appointment service must charge exactly the
--             precio_unitario agreed when the service was scheduled.
--          3. total must equal subtotal - descuento, and descuento <= subtotal.
--          4. created_by must match auth.uid() (cannot attribute payments to
--             someone else). Service-role/scripts (auth.uid() IS NULL) exempt.
--          5. Elimina la sobrecarga obsoleta de 10 parametros (sin p_nota):
--             la app siempre llama la version con p_nota; mantener ambas deja
--             una via sin validar y confunde a PostgREST.
-- Depends on: 014_update_payment_rpc.sql, 050_payment_nota.sql, 008_services_catalog.sql

-- Eliminar la sobrecarga vieja sin p_nota (quedaria sin las validaciones nuevas)
DROP FUNCTION IF EXISTS public.create_payment_with_invoice(UUID, DECIMAL, DECIMAL, TEXT, DECIMAL, UUID, JSONB, JSONB, UUID[], UUID);
