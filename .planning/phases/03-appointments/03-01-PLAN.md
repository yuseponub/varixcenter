...
must_haves:
  ...
  artifacts:
    - path: "package.json"
      provides: "FullCalendar dependencies"
...
<objective>
Install FullCalendar packages and create appointments database schema with PostgreSQL exclusion constraint for overlap prevention.

Purpose: Establish the foundation for the appointments feature - both npm dependencies and database structure must exist before any components or queries can be built.
Output: FullCalendar packages installed, appointments table created with RLS and btree_gist exclusion constraint.
</objective>
...
<task type="auto">
  <name>Task 1: Install FullCalendar npm packages</name>
  <files>package.json</files>
  <action>
Install FullCalendar v6.1.x packages as specified in 03-RESEARCH.md:

```bash
npm install @fullcalendar/react@^6.1.20 @fullcalendar/core@^6.1.20 @fullcalendar/timegrid@^6.1.20 @fullcalendar/interaction@^6.1.20 @fullcalendar/daygrid@^6.1.20
```
...
- `npm run build` passes (no import errors)
  ...
  <done>FullCalendar packages installed in package.json dependencies section</done>
</task>
...
3. Migration file exists with btree_gist extension and exclusion constraint
4. Migration includes RLS policies for all staff roles
</verification>
