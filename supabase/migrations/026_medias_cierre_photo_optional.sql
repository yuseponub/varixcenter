-- Migration: 026_medias_cierre_photo_optional.sql
-- Purpose: Make cierre_photo_path optional for medias cierres
-- Phase: 12-cash-closing-medias (adjustment)

-- 1. Alter the column to be nullable
ALTER TABLE public.medias_cierres
ALTER COLUMN cierre_photo_path DROP NOT NULL;

-- 2. Update the RPC to allow null photo
...
BEGIN
  ...
  INSERT INTO public.medias_cierres (
    ...
    p_cierre_photo_path,  -- Can be NULL now
    'cerrado',
    NULLIF(TRIM(p_notas), ''),
    v_user_id
  )
...
COMMENT ON FUNCTION public.create_medias_cierre IS
  'Crea cierre de caja de medias atomico. Tolerancia CERO para diferencias. Foto opcional.';
