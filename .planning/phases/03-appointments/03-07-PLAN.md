...
<task type="auto">
  <name>Task 1: Create appointment form component</name>
  <files>src/components/appointments/appointment-form.tsx</files>
  <action>
Create `src/components/appointments/appointment-form.tsx` following the pattern from patient-form.tsx:
...
Note: This requires Textarea component from shadcn/ui. If not installed, use Input with multiline or add it. Let me check - it's likely not installed yet. We'll add it or use a regular textarea.

Actually, let's add the Textarea component via shadcn CLI as part of this task, or fall back to a regular HTML textarea with Tailwind classes.
  </action>
  <verify>
...
export default async function NuevaCitaPage({ searchParams }: NuevaCitaPageProps) {
  ...
  // In a real app with many patients, this would be a search component
  // For now, fetch recent patients (limited to 100)
  const { patients } = await searchPatients({ q: '', page: 1, limit: 100 })
...
Also, let's check if Textarea component exists. If not, let's add it by running the shadcn CLI or creating a simple one.
  </action>
  <verify>
...
3. Status transitions via dialog (APT-03)
...
7. Test drag-and-drop:
   ...
   - Verify time updates (or shows error if overlap)
...
- APT-03: Status transitions work through dialog
