-- Migration: 050_payment_nota.sql
-- Purpose: Add optional nota (description/note) field to payments table
...
COMMENT ON COLUMN public.payments.nota IS 'Nota o descripcion libre del pago (opcional)';

-- ============================================================================
-- 2. UPDATE create_payment_with_invoice RPC to accept p_nota
-- ============================================================================
...
BEGIN
  ...
  INSERT INTO payments (
    patient_id, numero_factura, subtotal, descuento,
    descuento_justificacion, total, created_by, appointment_id, nota
  ) VALUES (
    p_patient_id, v_invoice_number, p_subtotal, p_descuento,
    NULLIF(TRIM(p_descuento_justificacion), ''), p_total, p_created_by,
    p_appointment_id, NULLIF(TRIM(p_nota), '')
  )
  ...
  LOOP
    INSERT INTO payment_items (
      payment_id, service_id, service_name, unit_price, quantity, subtotal
    ) VALUES (
      v_payment_id,
    ...
    IF v_item->>'appointment_service_id' IS NOT NULL THEN
      ...
      SET estado_pago = 'pagado', payment_item_id = v_payment_item_id, updated_at = now()
      WHERE id = v_apt_service_id AND estado_pago = 'pendiente';
  ...
  LOOP
    INSERT INTO payment_methods (payment_id, metodo, monto, comprobante_path)
    VALUES (
      v_payment_id,
...
GRANT EXECUTE ON FUNCTION public.create_payment_with_invoice(UUID, DECIMAL, DECIMAL, TEXT, DECIMAL, UUID, JSONB, JSONB, UUID[], UUID, TEXT) TO authenticated;

-- Also keep old signature working (without p_nota) for backwards compatibility
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'payments' AND column_name = 'nota'
  ) THEN
    RAISE EXCEPTION 'nota column not added to payments table';
  END IF;
  RAISE NOTICE 'Migration 050: payments.nota column added successfully';
END $$;
